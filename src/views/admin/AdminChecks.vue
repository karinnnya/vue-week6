  ...
  <table class="table mt-4 table-hover text-center align-middle">
    ...
    <tbody>
      ...
      <tr v-for="item in productOrders" :key="item.id">
        ...
        <td>
          <button type="button" class="btn btn-outline-primary btn-sm" @click="checkRevise(item)">
            編輯
          </button>
        </td>
        <td>
          <button type="button" class="btn btn-outline-secondary btn-sm" @click="openDe(item)">
            詳細資訊
          </button>
        </td>
...
export default {
  ...
  methods: {
      ...
      const { value: selectedStatus } = await Swal.fire({
        ...
        inputValidator: (value) => {
          return new Promise((resolve) => {
            axios.patch(`${VITE_APP_URL}/productOrders/${i.id}`, { Status: value })
              .then((res) => {
    ...
    getUser(i) {
      axios.get(`${VITE_APP_URL}/users/${i}`)
        .then((res) => {
    ...
    getChecks() {
      axios.get(`${VITE_APP_URL}/productOrders`)
        .then((res) => {
